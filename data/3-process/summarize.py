...
def call_llm(base_url: str, api_key: str, model: str, prompt: str) -> str:
    ...
    body = json.dumps({
        "model": model,
        "messages": [
            {"role": "system", "content": "You are a tech news editor. Write concise bilingual summaries. Output ONLY valid JSON."},
...
def build_prompt(items: list[dict]) -> str:
    ...
    for i, item in enumerate(items):
        ...
        lines.append(f"{i+1}. id={item['id']} | title={item['title']} | desc={desc}")
    return f"""For each item below, write a 2-3 sentence summary in Chinese (summary_zh) and English (summary_en).
Each summary should be ~100-150 Chinese characters / ~100 English words.
Explain WHY it matters to developers. Be informative and concise.
...
def main():
    ...
    if provider:
        print(f"[SUM] Using {provider['name']} API...")
    ...
    llm_count = len([s for s in summaries.values() if not s["summary_zh"].startswith("HN 热门") and not s["summary_zh"].startswith("开源项目")])
